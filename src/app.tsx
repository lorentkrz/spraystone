...
function App() {
  ...
  const generateImagePrompt = (options?: {
    ...
    const finishMap: Record<string, string> = {
      ...
      other:
        "Custom Spraystone blend mixing warm sandstone and cool limestone particles for a bespoke upscale look",
    };

    const finishDescription =
      finishMap[formData.finish as keyof typeof finishMap] ||
      finishMap["natural-stone"];
...
Keep it SHORT, practical, and focused on the visual transformation and pricing. No long letters or formalities.`;
  ...
  const generateImageFromUpload = async (
    ...
    try {
      ...
      const materialReferenceDataUrl = await fetchReferenceDataUrl(
        formData.finish || "natural-stone"
      );
      const materialReferenceMimeType = getMimeFromDataUrl(
        materialReferenceDataUrl
      );
      ...
      if (IMAGE_PROVIDER === "azure-openai") {
        ...
        try {
          ...
          const editsJson = await withRetry(
            ...
            },
            {
              ...
              onRetry: (attempt, total) =>
                reportProgress(
                  `Refining material layers... retry ${attempt}/${total}`,
                  52
                ),
            }
          );
          ...
          console.warn(
            "Azure edits failed, falling back to generations:",
            editsErr
          );
        }
        ...
        if (!b64) {
          ...
          const genJson = await withRetry(
            ...
            },
            {
              ...
              onRetry: (attempt, total) =>
                reportProgress(
                  `Calibrating finish... retry ${attempt}/${total}`,
                  65
                ),
            }
          );
        ...
        form.append(
          "image[]",
          uploadedImage,
          uploadedImage.name || "facade.png"
        );
        if (materialReferenceBase64) {
          const referenceBlob = base64ToBlob(
        ...
        const data = await withRetry(
          ...
          },
          {
            ...
            onRetry: (attempt, total) =>
              reportProgress(
                `Crafting Spraystone texture... retry ${attempt}/${total}`,
                72
              ),
          }
        );
        ...
        const resp = await withRetry(
          async () => {
            ...
            if (!response.ok) {
              throw new Error(
                `Proxy ${response.status}: ${await response.text()}`
              );
            }
            return response.json();
          },
          {
            ...
            onRetry: (attempt, total) =>
              reportProgress(
                `AI artisans polishing stone... retry ${attempt}/${total}`,
                82
              ),
          }
        );
        ...
        const result = await withRetry(
          () => imageModel.generateContent(promptParts),
          {
            ...
            onRetry: (attempt, total) =>
              reportProgress(
                `Gemini studio is iterating... retry ${attempt}/${total}`,
                78
              ),
          }
        );
        ...
        if (!AZURE_CHAT_COMPLETIONS_ENDPOINT || !AZURE_OPENAI_API_KEY) {
          ...
          const chatJson = await withRetry(
            ...
            },
            {
              ...
              onRetry: (attempt, total) =>
                setLoadingProgress(
                  `Analyzing facade... retry ${attempt + 1}/${total}`
                ),
            }
          );
    ...
    switch (currentStep) {
      ...
      case 7:
        return (
          <Step7Treatments
            formData={formData}
            onTreatmentChange={(id: string) =>
              handleTreatmentChange(id as Treatment | "__clear_all__")
            }
          />
        );
      case 8:
      ...
      <div className="mx-auto w-full max-w-5xl">
        ...
        <div className="relative overflow-hidden rounded-3xl border border-[#d4a574]/40 bg-white shadow-2xl">
          ...
          <div className="relative p-5 sm:p-8 lg:p-10">
            ...
            <div
              key={currentStep}
              className={`mx-auto mb-8 w-full max-w-4xl step-panel ${stepAnimationClass}`}
            >
              {renderStep()}
            </div>

            {error && (
              <div className="mb-6 rounded-xl border-2 border-red-200 bg-red-50 px-4 py-3 text-red-700">
                {error}
              </div>
            )}
